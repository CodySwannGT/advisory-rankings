...
describe("resource pagination helpers", () => {
  it("round-trips cursors", () => {
    ...
    expect(resources.decodeCursor(cursor)).toEqual({ sortKey: "smith", id: "abc-123" });
    expect(resources.decodeCursor("not-a-real-cursor!!!")?.sortKey).toBeUndefined();
  });
  ...
  it("walks all rows without skips or duplicates", () => {
    ...
    let pages = 0;
    while (true) {
      const res = resources.paginate(rows, { cursor: resources.decodeCursor(cursor), limit: 50 }, (r: any) => r.k);
  ...
  it("uses id tie-breaks and clamps limits", () => {
    ...
    const first = resources.paginate(rows, { cursor: null, limit: 2 }, (r: any) => r.k);
    const second = resources.paginate(rows, { cursor: resources.decodeCursor(first.nextCursor), limit: 2 }, (r: any) => r.k);
    expect([...first.items, ...second.items].map((r: any) => r.id)).toEqual(["a", "b", "c", "d"]);
    expect(resources.parsePagination({ get: () => "500" })).toEqual({ cursor: "500", limit: 100 });
  });
