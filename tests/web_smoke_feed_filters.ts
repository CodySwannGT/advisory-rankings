...
async function feedCategoryWithoutMoves(
  ...
  return await page.evaluate(async (selector: string) => {
    ...
    for (const category of options) {
      const data = await fetchCategoryFeed(category);
      if (feedItemCount(data) === 0) {
        return category;
      }
    }
    return "";

    async function fetchCategoryFeed(category: string): Promise<FeedResponse> {
      const response = await fetch(
        `/Feed?mode=recruiting&category=${encodeURIComponent(category)}&limit=1`
      );
      if (!response.ok) throw new Error(categoryProbeError(category, response));
      return (await response.json()) as FeedResponse;
    }

    function categoryProbeError(category: string, response: Response): string {
      return `feed category probe: /Feed?mode=recruiting&category=${category} -> ${response.status}`;
    }

    function feedItemCount(data: FeedResponse): number {
