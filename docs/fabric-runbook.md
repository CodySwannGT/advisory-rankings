...
fabric-deploy branch layout (commit a03f495):
  ...
  firms/ / advisors/ / teams/ + seo_shell.js
                        ← clean URL Fastify routes for directories and profiles
...
> `teams.html`) and `login.html`. Clean routes (`/firms`,
> `/advisors`, `/teams`, and `/<kind>/<slug>-<id>`) are Fastify shells
> that serve those same HTML files. Each page is a thin shell that
