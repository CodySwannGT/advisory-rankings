...
- Harper static serving is mounted at `/_static` so its wildcard miss handler
  does not own root URL misses. `static-web/index.js` registers exact root and
  asset routes for the generated web files, then uses Fastify's not-found
  handler to serve `web/404.html` only for browser document misses. Deployed
  recovery showed the explicit root asset routes are needed to avoid HTTP 500
  responses on misses after the unknown-route experiments.
...
fabric-deploy branch layout (commit a03f495):
  ...
  branches/ / firms/ / advisors/ / teams/ / articles/ / data-coverage/ / recruiting/ / rankings/ / regulatory/ / regulatory-discrepancies/ / corrections/ / compare/ / report-packet/ / watchlists/ / login/ / favicon/ + seo_shell.js
...
> `teams.html`), `coverage.html`, `recruiting.html`, `rankings.html`,
> `regulatory.html`, `compare.html`, `correction-inbox.html`, and
> `report-packet.html`.
> Clean routes (`/firms`, `/advisors`, `/teams`, `/coverage`, `/recruiting`, `/rankings`, `/regulatory`, `/compare`,
...
> Harper static is mounted at `/_static`; root assets from `web/**` (`/app.css`,
> generated `/*.js`, and nested `design-system/*`) are served by explicit
> `static-web/index.js` routes. Do not remove those routes as duplicate-looking
> code without deployed replay. Earlier deployed attempts showed the root static
> wildcard miss handler consumed top-level unknown routes before
> `setNotFoundHandler`, a single-segment `/:unknownRoute`, the `/*` wildcard, or
> the root `*` wildcard could run, even though direct `/404.html` assets were
> live. A later `wildcard: false` attempt, followed by removing explicit
> static-web asset routes, made deployed misses return HTTP 500 `Cannot read
> properties of undefined (reading 'length')`; keep explicit root asset routes
> and the non-root static mount until Harper's missing-document path is proven
> safe another way.
