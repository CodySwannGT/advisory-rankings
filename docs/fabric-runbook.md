...
only after `npm run build`. The primary deploy path is now the
GitHub Actions / `npm run deploy` push path in §6, which builds on
...
fabric-deploy branch layout (commit a03f495):
  ...
  resources.js          ← generated by npm run build from src/harper/resources.ts
...
| `npm run seed` (operations API for `upsert`) | 9925 | ❌ | `npm run seed:rest` (§7) |
| `npm run verify` (operations API for `sql`) | 9925 | ❌ | `npm run verify:rest` (§7) |
...
Edit `src/harper/resources.ts`, run `npm run build`, then deploy the
...
deploy together, which is the default because `npm run deploy` builds
...
live at `tests/resources_pagination.test.ts` — run `npm run test` before
...
`npm run deploy` runs `npm run build`, then `src/scripts/deploy.ts`
...
| 2026-05-21 | TypeScript migration. `src/` is now source of truth; `harper-app/resources.js` and `harper-app/web/**/*.js` are generated by `npm run build` and ignored by git. Deploy workflow now runs `npm run deploy`, which builds before packaging `harper-app/`. | OK |
...
(`npm run smoke`, backed by `tests/web_smoke.ts`) against the live cluster URL. Required repo
secrets:
...
are sandbox-friendly equivalents of `npm run seed` / `npm run verify`.
...
npm run seed:rest                    # PUTs each canonical record via /<TableName>/<id>
npm run verify:rest                  # GETs each table and joins client-side
```
...
- `seed_via_rest.ts` — loads `src/data/seed-data.json` and calls
  ...
  upsert-by-id in Harper. Output matches `npm run seed` exactly:
...
`npm run brokercheck --` populates `Advisor.finraCrd`,
...
npm run brokercheck -- --enrich --max 20

# Add a firm-level Regulatory record card:
npm run brokercheck -- --firm-id 19616

# Discover net-new advisors at a known firm:
npm run brokercheck -- --firm-roster 47770 --max 50
```
...
`src/scripts/preview_feed.ts` (a.k.a. `npm run preview`) renders the
...
Harper's built-in `static:` extension. `npm run build` emits the
...
`npm run preview` (a.k.a. `node dist/src/scripts/preview_feed.js` after build); it
