...
- Explicit static web Fastify routes from `static-web/index.js` for `/`,
  each built `web/**` asset, and the unknown-document fallback. These own the
  URL paths the HTML shells reference, so `/app.css`, generated root JS
  modules, nested `design-system/` files, and recoverable unknown document
  routes all use the same route layer without wildcarding API resources.
...
fabric-deploy branch layout (commit a03f495):
  ...
  firms/ / advisors/ / teams/ / articles/ / recruiting/ / rankings/ / regulatory/ / regulatory-discrepancies/ / corrections/ / compare/ / report-packet/ / watchlists/ / login/ / favicon/ + seo_shell.js
...
> `compare.html`, `correction-inbox.html`, and `report-packet.html`.
...
> `static-web/index.js` registers `/`, exact built-asset routes from `web/**`
> (`/app.css`, generated `/*.js`, and nested `design-system/*`), and the
> recoverable unknown-document `/*` route. `config.yaml` intentionally omits
> Harper's built-in `static:` extension because deployed replay showed that the
> static extension's root miss handler returned bare `Not found` before
> Fastify fallback routes could serve `404.html`.
...
> **Static route ownership caveat — symptom: unknown document routes return
> bare `Not found`.** The built-in `static:` extension handles root misses
> before Fastify fallback routes. Keep web serving in `static-web/index.js` so
> exact assets and the `/*` document fallback share one route layer. The old
> `static.files: 'web/*'` non-recursive glob also 404'd nested
> `web/design-system/` assets on 2026-05-02; recursive asset discovery in
> `static-web/index.js` covers those files now.
