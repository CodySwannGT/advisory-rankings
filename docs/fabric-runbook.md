...
| `/mcp-gallery` and `/developers/mcp` (`mcp-gallery.html` still works) | Public MCP gallery: endpoint/server metadata, read-only tool and resource-template inventory, private-data boundary copy, stale/fresh status, and explicit unavailable states from `/McpCatalog`. |
