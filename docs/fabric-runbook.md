...
- Harper static serving owns `/`, built `web/**` assets, and its own wildcard
  miss handler. Fabric currently ignores this component's `fastifyRoutes`
  surface, so `bun run build` generates static directory `index.html` shells
  for clean document routes such as `/advisors`, `/investor-proof`, and
  `/recruiting/shortlist`. Deploy verification probes those clean document
  shells after every upload so a stale or missing static shell cannot pass on
  `/version.js` alone. Top-level unknown routes currently return Harper's bare
...
> `/watchlists`, `/corrections`, and `/login`) are static directory
> `index.html` shell files generated by
> `bun run build` from the corresponding `.html` shells. The historical Fastify
> shell modules remain in the component tree, but deployed Fabric behavior shows
> `fastifyRoutes` is not a reliable route surface for this app. Profile clean
> profile clean paths still need a separate deployed fix if Fastify remains
> unavailable.
