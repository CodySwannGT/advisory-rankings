...
- Harper static is mounted at `/`, serving built `web/**` assets and generated
  directory-index shells for clean top-level document routes such as
  `/advisors/`, `/investor-proof/`, `/source-triage/`, and
  `/recruiting/shortlist/`. Clean dynamic profile/article URLs
  (`/advisors/<id>`, `/firms/<id>`, `/teams/<id>`, `/articles/<id>`) are
  exported as lowercase Harper `jsResource` classes in
  `resource-clean-web-routes.ts` with `directURLMapping = true`. Deploy
  verification probes both top-level and dynamic clean document routes after
  every upload so a stale, partial, or missing route registration cannot pass on
  `/version.js` alone.
...
fabric-deploy branch layout (commit a03f495):
  ...
  branches/ / firms/ / advisors/ / teams/ / articles/ / data-coverage/ / investor-proof/ / recruiting/ / recruiting-shortlist/ / rankings/ / regulatory/ / regulatory-discrepancies/ / corrections/ / compare/ / report-packet/ / watchlists/ / login/ / favicon/ + seo_shell.js
                        ← legacy Fastify shell routes kept in the package
  web/                  ← Facebook-style UI and generated clean top-level directory shells
...
> `recruiting-shortlist.html`, `rankings.html`, `regulatory.html`,
> `compare.html`, `correction-inbox.html`, and
> `report-packet.html`.
> Clean top-level routes (`/firms`, `/advisors`, `/teams`, `/coverage`,
> `/investor-proof`, `/recruiting`, `/recruiting/shortlist`, `/rankings`,
> `/regulatory`, `/regulatory/discrepancies`, `/compare`, `/report-packet`,
> `/watchlists`, `/corrections`, `/source-triage`, and `/login`) are generated
> static directory `index.html` shell files under `web/`. Dynamic clean paths
> such as `/advisors/<id>`, `/firms/<id>`, `/teams/<id>`, and `/articles/<id>`
> are Harper `jsResource` web-shell resources.
> `/login.html` is intentionally not generated.
> Harper static is mounted at `/`; root assets from `web/**` (`/app.css`,
> generated `/*.js`, and nested `design-system/*`) stay available even when
> Fabric does not mount `fastifyRoutes`. The historical Fastify route modules
> remain packaged, but they are no longer the sole clean-route mechanism.
