...
| `/PublicBranches` | `q` matches branch name, building, address, city, state, or firm name substrings; `firm` matches firm id or name substrings; `gapGroup` exactly matches the public branch gap group (`loaded`, `partial`, `unavailable`, `zero-advisor`, or `missing-source`); `state` exactly matches `Branch.state`; `city` and `market` match city/name/building/address substrings; `sourceType` exactly matches materialized `BranchCoverage.sourceTypes`; `level` exactly matches `Branch.level`; `minAdvisorCount` filters on materialized current advisor count. |
...
| `GET /DataCoverage` | ✅ 200 | **Decision: intentionally public.** Public dashboard payload for `/coverage`: aggregate entity counts, public resource probes, rankings/recruiting gaps, research freshness, source-table context, and limitations. It reports public-resource provenance only; it does not expose private user rows, analyst workflow rows, or secrets. |
| `GET /AdvisorResearchQueue?limit=…` | ✅ 200 | **Decision: intentionally public.** Public-safe research-work queue rows plus priority groups: advisor identity, firm context, source/check status, missing public profile fields, profile URLs, returned-slice counts, and group filter mappings. No private user tables or analyst workflow rows are loaded. |
| `GET /SourceArticleTriage?category=…&reason=…&limit=…` | ✅ 200 | **Decision: intentionally public.** Public source-article extraction-gap queue: article metadata, source and ArticleView links, public entity/event counts, body/provenance state, and stable reason tokens. It reads public Article/FieldAssertion evidence and omits private analyst/user rows. |
| `GET /RecruitingDealDataGaps?limit=…` | ✅ 200 | **Decision: intentionally public.** Public recruiting move coverage gaps: missing deal-data labels, source-status flags, provenance ids, and public follow-up links for article, firm, advisor/team, and recruiting-market context. It narrows public recruiting data only; it does not expose private watchlists, ratings, correction rows, analyst assignments, or reviewer notes. |
...
| `GET /PublicBranches` | `PublicBranches` | Branch rows joined to firm names and materialized `BranchCoverage` rows for source metadata and distinct current advisor counts. |
