...
| `/AdvisorCorrectionRequest` | POST | `allowCreate=true`, session required in handler | Creates a pending correction request row for signed-in users without changing source-backed advisor facts. Body includes advisor id, field name, displayed/proposed values, submitter note, and source context. |
...
| `POST /AdvisorCorrectionRequest`, `GET/POST /AdvisorCorrectionRequest/<id>` | ❌ anonymous / ✅ session | The scoped resource allows correction submission and review only after `getCurrentUser()` resolves a signed-in user; source-backed `AdvisorProfile` fields are not mutated by request writes. |
