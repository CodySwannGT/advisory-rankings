...
- Harper static serving owns `/` and the built `web/**` assets with
  `wildcard: false`, which keeps `/app.css`, generated root JS modules, and
  nested `design-system/` files on exact static routes while letting missing
  document paths fall through to the app-level not-found handler instead of
  being consumed by the static plugin's bare `Not found` response.
...
fabric-deploy branch layout (commit a03f495):
  config.yaml           ← lifted from harper-app/, includes
                          ...
                          fastifyRoutes +
                          static: { files: 'web/**', wildcard: false }
  ...
  firms/ / advisors/ / teams/ / articles/ / data-coverage/ / recruiting/ / rankings/ / regulatory/ / regulatory-discrepancies/ / corrections/ / compare/ / report-packet/ / watchlists/ / login/ / favicon/ + seo_shell.js
...
> Harper static serves `/` and exact built assets from `web/**` (`/app.css`,
> generated `/*.js`, and nested `design-system/*`) with `wildcard: false`.
> Earlier deployed attempts showed the static wildcard miss handler consumed
> top-level unknown routes before `setNotFoundHandler`, a single-segment
> `/:unknownRoute`, the `/*` wildcard, or the root `*` wildcard could run, even
> though direct `/404.html` assets were live. `static-web/index.js` owns only
> the document fallback and serves the HTML shell for document requests with
