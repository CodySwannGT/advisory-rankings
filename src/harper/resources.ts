 ...
 * to ~15 raw REST endpoints (the verify_via_rest.ts pattern):
 ...
 *    means re-implementing them in both the browser and resource layer.
	...
	async get(target) {
		...
		for (const e of db.employments) {
			...
			rows.push({
				...
				_id: e.id || a.id,
				advisor: { id: a.id, name: advisorDisplayName(a), headshotUrl: a.headshotUrl || null, careerStatus: a.careerStatus },
