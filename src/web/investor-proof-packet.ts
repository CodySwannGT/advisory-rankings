...
function loadPacket(center: HTMLElement, right: HTMLElement): void {
  api<InvestorProofPacketResponse>(PACKET_RESOURCE)
    .then(packet => renderPacket(packet, center, right))
...
function metricCard(metric: DataCoverageMetric): HTMLElement {
  return el(
    ...
    el(
      ...
      `${metric.source}: ${metric.publicResource}`
    ),
    metric.limitation
...
function linkRow(link: InvestorProofLink): HTMLElement {
  return el(
    ...
    el("span", {}, `${link.publicResource} from ${link.sourceTable}`),
...
function provenanceCard(packet: InvestorProofPacketResponse): HTMLElement {
  return SectionCard({
    ...
    body: DetailsCard({
      title: "Packet inputs",
      pairs: [
        ["Packet resource", PACKET_RESOURCE],
        ["Generated", fmtDate(packet.generatedAt)],
        ["Resources", provenanceValue(packet.provenance.publicResources)],
        ["Source tables", provenanceValue(packet.provenance.sourceTables)],
      ],
    }),
  });
}
