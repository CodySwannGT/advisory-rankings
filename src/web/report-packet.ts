...
import { firmName } from "./compare-sections.js";
import {
  ...
  return SectionCardComponent({
    ...
    body: [
      ...
      el(
        ...
        ...payload.items.map(packetAdvisor)
      ),
    ],
  });
}
 ...
 */
function packetAdvisor(item: AdvisorComparisonItem): HTMLElement {
  return el(
    "article",
    {
    ...
    el("p", {}, item.status === "found" ? firmName(item) : "Advisor not found")
  );
}
