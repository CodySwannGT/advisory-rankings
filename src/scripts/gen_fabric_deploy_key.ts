...
//   ~/.cache/harper-signup/fabric-deploy-key
//   ~/.cache/harper-signup/fabric-deploy-key.pub
//
...
const OUT_DIR =
  (argOut && argOut.slice("--out=".length)) ||
  process.env.OUT_DIR ||
  path.join(os.homedir(), ".cache", "harper-signup");
const COMMENT =
  process.env.KEY_COMMENT || "harper-fabric-deploy@advisory-rankings";
...
/**
 * Encodes OpenSSH length-prefixed string fields.
...
const padding = Buffer.from(
  Array.from({ length: padLen }, (_value, index) => index + 1)
);
...
console.log();
console.log(
  "public key line (paste into GitHub Settings → Deploy keys → Add deploy key):"
);
console.log(pubLine.trimEnd());
console.log();
console.log(
  "private key (paste into Fabric Studio → Config → SSH Keys → Add):"
);
