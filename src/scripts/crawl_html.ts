...
/**
 * Delays HTML crawl requests so repeated research pulls stay polite.
 * @param ms - Delay duration in milliseconds.
 * @returns Resolves after the requested delay.
 */
...
for (const url of urls) {
  const res = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 advisory-rankings-research/0.1" },
  });
