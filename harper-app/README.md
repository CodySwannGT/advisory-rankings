...
| File | Purpose |
|---|---|
| `config.yaml` | Component config — points Harper at `*.graphql` for the schema, enables REST, loads `resources.js` as a `jsResource`, registers clean URL and favicon Fastify routes, and serves `web/**` as a static site with `web/404.html` as the recoverable static miss page. |
