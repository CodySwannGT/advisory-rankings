...
| `firms/`, `advisors/`, `teams/`, `articles/`, `recruiting/`, `rankings/`, `regulatory/`, `compare/`, `seo_shell.js` | Fastify route shells for SEO-friendly URLs. `/firms`, `/advisors`, `/teams`, `/recruiting`, `/rankings`, `/regulatory`, and `/compare` serve page HTML; `/firms/<slug>-<id>`, `/advisors/<slug>-<id>`, `/teams/<slug>-<id>`, and `/articles/<slug>-<id>` serve the matching detail shell. |
...
- **Web UI** served at `http://127.0.0.1:9926/` from `web/index.html`.
  ...
  `/compare?ids=<advisorId>,<advisorId>`;
