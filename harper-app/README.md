...
| `branches/`, `firms/`, `advisors/`, `teams/`, `articles/`, `data-coverage/`, `investor-proof/`, `recruiting/`, `recruiting-shortlist/`, `rankings/`, `regulatory/`, `corrections/`, `compare/`, `report-packet/`, `seo_shell.js` | Historical Fastify route shells for SEO-friendly URLs. Fabric currently ignores this config surface for this component, so build-generated static shells under `web/` are the deployed clean-route path for top-level pages. |
| `web/` | AdvisorBook static SPA. HTML/CSS are tracked here; browser `.js` modules and clean-route directory shells such as `advisors/index.html`, `investor-proof/index.html`, and `recruiting/shortlist/index.html` are generated by `bun run build`. UI is composed from the Atomic Design library under `src/web/design-system/` and emitted to `web/design-system/` — see `docs/design-system.md`. |
